class Api::V1::DepartmentsController < Api::V1::BaseController
  before_action :set_department, only: [ :delete, :update, :show ]
  before_action :check_authorize

  def departments_trees
      ...
      @departments = @q.result().paginate(page: params[:page], per_page: per_page) # , :total_entries =>  Department.total_entries)
  ...
  def index
    if params[:list]
      ...
      @departments = @q.paginate(page: params[:current], per_page: params[:size]) # , :total_entries =>  Department.total_entries)
      gen_log(@current_user.id, @current_ip,  params[:action], 1, "访问部门列表")
  ...
  def create
    ...
    if @department.save
      ...
      render json: { code: 200, success: 1, msg: t("create_success"), data: @department }
    else
  ...
  def show
    render json: { code: 200, success: 1, msg: "ok", data: @department }
  end
  ...
  private
    ...
    def check_authorize
      begin
        authorize!(params[:action].to_sym, @department || Department)
