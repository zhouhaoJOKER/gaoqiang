class Mfg::ScrapOrder < ApplicationRecord

    # columns: code, scrap_category, product_id, item_id, lot_id, location_id, planned_qty, status, user_id, updater_id, created_at, updated_at
    attribute :created_time, :string
    attribute :updated_time, :string

    has_one_attached :file
    has_many_attached :files

    belongs_to :user, class_name: "User", foreign_key: "user_id", optional: true
    belongs_to :updater, class_name: "User", foreign_key: "updater_id", optional: true
    belongs_to :product, class_name: "Mat::Product", foreign_key: "product_id", optional: true
    belongs_to :item, class_name: "Mat::Item", foreign_key: "item_id", optional: true
    belongs_to :lot, class_name: "Inv::Lot", foreign_key: "lot_id", optional: true
    belongs_to :location, class_name: "Wms::Location", foreign_key: "location_id", optional: true
    has_many :logs, class_name: "Log", as: :record
    has_many :activities, class_name: "Activity", as: :record

    def base_info
        {
            id: id,
            code: code,
            scrap_category: scrap_category,
            product_id: product_id,
            product_name: (product.name rescue ""),
            item_id: item_id,
            item_name: (item.name rescue ""),
            lot_id: lot_id,
            lot_code: (lot.code rescue ""),
            location_id: location_id,
            planned_qty: planned_qty,
            status: status,
            user_id: user_id,
            user_name: (user.name rescue ""),
            updater_id: updater_id,
            updater_name: (updater.name rescue ""),
            created_at: created_at,
            updated_at: updated_at,
            created_time: created_time,
            updated_time: updated_time,
        }
    end

    def show_info
        {
        id: id,
        code: code,
        scrap_category: scrap_category,
        }
    end
    

    def self.imp_exp_hander
        %w(code  scrap_category  product_id  item_id  lot_id  location_id  planned_qty  status  user_id  updater_id  created_at  updated_at)
    end

    def sum_amount(column, amount)
        update_columns("#{column}": amount)
    end

    # 报废单完成时同步库存：减少对应货位数量
    def sync_inventory_on_complete!(operator_id: nil)
        return false unless Inv::InventoryService.completed?(status)
        return false if inventory_synced == true
        return false unless item_id.present? && location_id.present? && planned_qty.to_d > 0

        operator_id ||= updater_id || user_id

        ActiveRecord::Base.transaction do
            Inv::InventoryService.decrease(
                item_id: item_id,
                location_id: location_id,
                quantity: planned_qty.to_d,
                related_order_type: "Mfg::ScrapOrder",
                related_order_id: id,
                operator_id: operator_id,
                remark: "报废单#{code}报废出库"
            )
            update_columns(inventory_synced: true)
            true
        end
    rescue => e
        Rails.logger.error "ScrapOrder#sync_inventory_on_complete! failed: #{e.message}"
        raise
    end
end
