class Mfg::BatchingLine < ApplicationRecord

    # columns: batching_id,sort,category,name,product_id,batch_number,qty_kg,content_percent,adjusted_qty_kg,remark,user_id,, created_at, updated_at
    # attr_accessor :sort
    # serialize :members
    ...
    belongs_to :batching, class_name: "Mfg::Batching", foreign_key: "batching_id", optional: true
    ...
    def base_info
        {
            ...
            batching_name: (batching.name rescue ""),
            sort: sort,
            category: category,
            name: name,
            ...
            adjusted_qty_kg: adjusted_qty_kg,
            remark: remark,
            user_id: user_id,
    ...
    #导入导出头部设置
    # Mfg::BatchingLine.imp_exp_hander
    def self.imp_exp_hander
        %w(batching_id  sort  category  name  product_id  batch_number  qty_kg  content_percent  adjusted_qty_kg  remark  user_id  created_at  updated_at)
    end

    # 示例计算金额
    # Mfg::BatchingLine.last.sum_amount("receive_amount", 1000)
...
# #Mfg/batching line batching_lines
