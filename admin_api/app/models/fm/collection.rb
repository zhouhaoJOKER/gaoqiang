class Fm::Collection < ApplicationRecord
    self.table_name = "fm_collections"

    # columns: contract_id,sort,product_id,name,unit_id,qty,price,amount_excl_tax,tax,amount,user_id,updater_id,remark,, created_at, updated_at
    # attr_accessor :sort
    # serialize :members
    ...
    #导入导出头部设置
    # Fm::Collection.imp_exp_hander
    def self.imp_exp_hander
        %w(contract_id  sort  product_id  name  unit_id  qty  price  amount_excl_tax  tax  amount  user_id  updater_id  remark  created_at  updated_at)
    end

    # 示例计算金额
    # Fm::Collection.last.sum_amount("receive_amount", 1000)
