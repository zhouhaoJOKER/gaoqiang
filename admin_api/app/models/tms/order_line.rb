class Tms::OrderLine < ApplicationRecord

    # columns: shipping_order_id,product_id,product_name,packaging_spec,pieces_capitalized,pieces,net_weight_display,net_weight_ton,unit_price,amount,sort,user_id,, created_at, updated_at
    # attr_accessor :sort
    # serialize :members
    ...
    belongs_to :shipping_order, class_name: "ShippingOrder", foreign_key: "shipping_order_id", optional: true
    ...
    def base_info
        {
            ...
            shipping_order_name: (shipping_order.name rescue ""),
            product_id: product_id,
            product_name: (product.name rescue ""),
            product_name: product_name,
            packaging_spec: packaging_spec,
            pieces_capitalized: pieces_capitalized,
            pieces: pieces,
    ...
    #导入导出头部设置
    # Tms::OrderLine.imp_exp_hander
    def self.imp_exp_hander
        %w(shipping_order_id  product_id  product_name  packaging_spec  pieces_capitalized  pieces  net_weight_display  net_weight_ton  unit_price  amount  sort  user_id  created_at  updated_at)
    end

    # 示例计算金额
    # Tms::OrderLine.last.sum_amount("receive_amount", 1000)
