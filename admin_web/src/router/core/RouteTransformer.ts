...
export class RouteTransformer {
  ...
  private handleIframeRoute(
    ...
    if (depth === 0) {
      ...
      const parentPath = this.extractFirstSegment(sourceRoute.path || '')
      targetRoute.path = parentPath
      targetRoute.name = ''

      // 提取子路由的相对路径
      const childPath = this.extractRelativePath(sourceRoute.path || '', parentPath)
  ...
  private handleFirstLevelRoute(
    ...
    const parentPath = this.extractFirstSegment(route.path || '')
    converted.path = parentPath
    converted.name = ''
    route.meta.isFirstLevel = true

    // 提取子路由的相对路径
    const childPath = this.extractRelativePath(route.path || '', parentPath)
  ...
  /**
   * 提取相对路径（从完整路径中移除父路径部分）
   */
  private extractRelativePath(fullPath: string, parentPath: string): string {
    ...
    // 如果完整路径以父路径开头，提取剩余部分
    if (cleanFullPath.startsWith(cleanParentPath)) {
      const relative = cleanFullPath.substring(cleanParentPath.length).replace(/^\/+/, '')
      return relative || '' // 如果为空，返回空字符串（表示与父路径相同）
    }

    // 如果不匹配，返回完整路径（可能是绝对路径）
    return fullPath
  }
}
