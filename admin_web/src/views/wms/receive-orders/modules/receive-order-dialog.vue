<template>
  <ElDialog
    ...
    @closed="handleClosed"
  >
    <div class="receive-order-dialog">
      <!-- 状态显示区域 -->
      <StatusStepper
        :id="form.id"
        :status="form.status"
...
<script setup lang="ts">
  ...
  import {
    createWmsReceiveOrderApi,
    updateWmsReceiveOrderApi,
    viewWmsReceiveOrderApi,
    listWmsReceiveOrderLineApi
  ...
  interface Emits {
    ...
    (e: 'created', receiveOrder: any): void
    (e: 'updated', receiveOrder: any): void
  }
  ...
  const isEditMode = computed(() => !!props.receiveOrder && !!props.receiveOrder.id)
  const dialogTitle = computed(() => {
    if (props.readonly) return '查看入库单'
    return isEditMode.value ? '编辑入库单' : '创建入库单'
  })
  ...
  const loadData = async (): Promise<void> => {
    if (!isEditMode.value || !props.receiveOrder?.id) return

    try {
      const res = await viewWmsReceiveOrderApi({ id: props.receiveOrder.id })
      ...
      // 加载明细行
      const linesRes = await listWmsReceiveOrderLineApi({ q: { receive_order_id_eq: data.id } })
  ...
  const handleSave = async (): Promise<void> => {
    ...
    await formRef.value.validate((valid) => {
      ...
      if (isEditMode.value) {
        updateWmsReceiveOrderApi(String(form.id!), submitData)
          .then((res) => {
        ...
        createWmsReceiveOrderApi(submitData)
          .then((res) => {
  ...
  const handleStatusChange = async (nextStatus: string): Promise<void> => {
    try {
      ...
      const res = await updateWmsReceiveOrderApi(String(form.id), submitData)
