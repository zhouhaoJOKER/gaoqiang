    ...
    <div class="return-order-dialog">
      ...
      >
        <ElRow :gutter="20">
          ...
          <ElCol :span="12">
            <ElFormItem label="退货类别" prop="return_category_id">
              <PropertySelectWithAdd
                v-model="form.return_category_id"
                sign="wms_return_category"
                placeholder="请选择退货类别"
              />
            </ElFormItem>
          </ElCol>
        ...
        <ElRow :gutter="20">
          <ElCol :span="12">
            <ElFormItem label="原单ID">
              <ElInput
                v-model="form.original_order_id"
                type="number"
                placeholder="请输入原单ID"
                :readonly="props.readonly"
              />
      ...
      <div class="lines-section">
        ...
        <ElTable :data="form.lines" border style="width: 100%">
          ...
          <ElTableColumn label="已退数量" min-width="120">
            <template #default="{ row }">
              <ElInput
                v-model="row.returned_qty"
                type="number"
                placeholder="0.00"
                :readonly="props.readonly"
              />
...
<script setup lang="ts">
  ...
  import {
    createWmsReturnOrderApi,
    updateWmsReturnOrderApi,
    viewWmsReturnOrderApi,
    listWmsReturnOrderLineApi
  ...
  interface Emits {
    ...
    (e: 'created', returnOrder: any): void
    (e: 'updated', returnOrder: any): void
  }
  ...
  const isEditMode = computed(() => !!props.returnOrder && !!props.returnOrder.id)
  const dialogTitle = computed(() => {
    if (props.readonly) return '查看退货单'
    return isEditMode.value ? '编辑退货单' : '创建退货单'
  })
  ...
  const rules = reactive<FormRules>({
    ...
    return_category_id: [{ required: true, message: '请选择退货类别', trigger: 'change' }],
  ...
  const form = reactive({
    id: undefined,
    code: '',
    return_category_id: null as string | number | null,
    return_category: '',
    status: 'pending',
    warehouse_id: undefined as string | number | undefined,
    plan_date: '',
    original_order_id: undefined as string | number | undefined,
    lines: [] as any[]
  })
  ...
  const loadData = async (): Promise<void> => {
    if (!isEditMode.value || !props.returnOrder?.id) return

    try {
      const res = await viewWmsReturnOrderApi({ id: props.returnOrder.id })
      const data = (res as any).data || {}
      Object.assign(form, {
        id: data.id,
        code: data.code || '',
        return_category_id: data.return_category_id ?? data.return_category ?? null,
      ...
      // 加载明细行
      const linesRes = await listWmsReturnOrderLineApi({ q: { return_order_id_eq: data.id } })
  ...
  const handleSave = async (): Promise<void> => {
    ...
    await formRef.value.validate((valid) => {
      ...
      if (isEditMode.value) {
        updateWmsReturnOrderApi(String(form.id!), submitData)
          .then((res) => {
        ...
        createWmsReturnOrderApi(submitData)
          .then((res) => {
  ...
  const handleStatusChange = async (nextStatus: string): Promise<void> => {
    try {
      ...
      const res = await updateWmsReturnOrderApi(String(form.id), submitData)
