...
<script setup lang="ts">
  ...
  const searchForm = ref({ name_cont: undefined, client_id_eq: undefined })
  const proTable = ref()
    ...
    core: {
      ...
      columnsFactory: () => [
        ...
        },
        {
          ...
          label: '客户',
          minWidth: 140,
          formatter: (row: unknown) => (row as ContactItem).client_name || '-'
        },
        {
          prop: 'dept',
          label: '部门',
        ...
        },
        {
          ...
          label: '职务',
          minWidth: 100,
          formatter: (row: unknown) => (row as ContactItem).title || '-'
        },
        {
          prop: 'phone',
          label: '电话',
  ...
  const handleView = (row?: ContactItem) => {
    if (row?.id) router.push({ path: `/crm/contacts/${row.id}` })
  }
  ...
  const deleteContact = (row: ContactItem) => {
    ElMessageBox.confirm(`确定删除联系人"${row.name || '未命名'}"吗？`, '删除确认', {
      ...
      .then(() =>
        deleteCrmContactApi(Number(row.id)).then(() => {
  ...
  const handleBatchDelete = () => {
    ...
    ElMessageBox.confirm(`确认删除选中的 ${selectedRows.value.length} 个联系人吗？`, '删除确认', {
      ...
      .then(async () => {
        await batchActionCrmContactApi({
          actions: 'delete',
  ...
  const handleExport = () => {
    ...
    exportExcel(columns.value, _data, '客户联系人数据')
