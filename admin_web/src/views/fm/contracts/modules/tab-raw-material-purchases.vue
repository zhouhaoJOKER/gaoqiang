<template>
  <div class="tab-raw-material-purchases">
    <div class="tab-toolbar">
      <ElButton type="primary" @click="handleAdd">
        <ArtSvgIcon icon="ri:add-line" class="mr-1" />新增原料采购
      </ElButton>
    </div>
    <div v-loading="loading" class="purchase-list">
      <template v-if="contractList.length === 0 && !loading">
        <ElEmpty description="暂无原料采购合同" />
      </template>
...
<script setup lang="ts">
  ...
  const fetchList = async (fmContractId: string | number) => {
    if (!fmContractId) return
    loading.value = true
    try {
      const res = await listPurContractApi({
        'q[contract_id_eq]': fmContractId,
        'q[sign_eq]': 'raw_material',
        per_page: 100
      })
...
<style scoped>
  .tab-raw-material-purchases .tab-toolbar {
    margin-bottom: 16px;
  }
