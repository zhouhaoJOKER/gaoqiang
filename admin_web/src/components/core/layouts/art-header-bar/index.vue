    ...
    >
      <div class="flex-c flex-1 min-w-0 leading-15" style="display: flex">
        ...
        <div class="flex-c c-p" @click="toHome" v-if="isTopMenu">
          ...
          <p v-if="width >= 1400" class="my-0 mx-2 ml-2 text-lg">{{ systemInfo.name }}</p>
        </div>
...
<script setup lang="ts">
  ...
  const {
    ...
    // shouldShowNotification / shouldShowChat：通知与聊天按钮在模板中注释，恢复时请一并解构
    // fastEnterMinWidth：快速入口注释时无需解构
  } = useHeaderBar()
  ...
  /**
   * 切换通知面板：恢复顶部通知按钮时取消模板注释并绑定 @click="visibleNotice"
   */
  ...
  /**
   * 打开聊天窗口：恢复顶部聊天按钮时取消模板注释并绑定 @click="openChat"
   */
  // const openChat = (): void => {
