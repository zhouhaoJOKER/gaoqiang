...
<script setup lang="ts">
  ...
  const toLogin = () => {
    // 清除锁屏状态
    localStorage.removeItem('screen-lock-status')
    userStore.logOut()
  }
  ...
  const openLockScreen = () => {
    // 直接设置锁屏状态，不显示弹窗
    userStore.setLockStatus(true)
    localStorage.setItem('screen-lock-status', 'true')
    // 设置一个默认的锁屏密码
    // 如果默认密码不存在就创建一个默认密码
    const lockPassword = userStore.lockPassword
    if (!lockPassword) {
      const defaultPassword = CryptoJS.AES.encrypt('123', ENCRYPT_KEY).toString()
