...
<template>
  <div class="art-table" :class="{ 'is-empty': isEmpty }">
    <ElTable
      ref="elTableRef"
      v-loading="!!loading"
...
<script setup lang="ts">
  ...
  const height = computed(() => {
    ...
    // 默认：给 ElTable 预留分页/表头空间（避免分页被裁剪）
    return containerHeight.value.height
  })
  ...
  // 是否显示分页器：只要传入 pagination 即显示（无数据时也展示，便于翻页/改每页条数）
   ...
   * 处理表格排序变化
   */
  const handleSortChange = (sort: { prop: string; order: 'asc' | 'desc' | null }) => {
    // 获取当前排序的列配置，检查是否有自定义的sortField
    const columnConfig = props.columns?.find((col) => col.prop === sort.prop)
    // 传递三个参数：当前字段、排序方向、自定义排序字段（如果有）
    emit('sort-change', sort.prop, sort.order, columnConfig?.sortField)
  }

  const emit = defineEmits<{
    (e: 'pagination:size-change', val: number): void
    (e: 'pagination:current-change', val: number): void
    (e: 'sort-change', prop: string, order: 'asc' | 'desc' | null, sortField?: string): void
  }>()
