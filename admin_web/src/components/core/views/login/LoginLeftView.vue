<!-- 登录、注册、忘记密码左侧背景 -->
<template>
  <div class="login-left-view" :style="{ backgroundImage: `url(${currentBackground})` }">
    <div class="logo">
      <ArtLogo class="icon" size="46" />
      <h1 class="title">{{ systemInfo.name }}</h1>
    </div>
    ...
    <div class="geometric-decorations">
      ...
      <!-- 叠加方块组 -->
      <div class="squares-group" @click="switchToRandomBackground()">
        <i
...
<script setup lang="ts">
  ...
  // 生成指定类别的图片链接
  const generateImageUrl = (category: string, number: number) => {
    return `https://wss-static.wenshushu.cn/images/background/${category}/${number}.jpg`
  }

  // 生成所有可能的背景图片链接
  const generateAllBackgroundImages = () => {
    const images: string[] = []
    backgroundCategories.forEach((category) => {
      for (let i = 3; i <= 20; i++) {
        images.push(generateImageUrl(category, i))
      }
    })
    return images
  }
  ...
  // 登录页默认首图（用户指定）
  const defaultLoginBanner = 'https://cdn.worldidc.cn/customer/ntgqbz/banner_cn.jpg'

  // 背景图片数组：首图使用指定图片，其余保留原有随机图库
  const backgroundImages = [defaultLoginBanner, ...generateAllBackgroundImages()]

  // 当前背景图片索引
  const currentBgIndex = ref(0)

  // 当前背景图片
  const currentBackground = computed(() => backgroundImages[currentBgIndex.value])

  // 随机切换背景图片
  const switchToRandomBackground = () => {
    console.log('切换背景图片')
